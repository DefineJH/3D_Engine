...
float4 main(float3 worldPos : Position , float3 n : Normal ) : SV_Target
{
	...
	float lightAngle = max(0.0f, dot(n, posToLight));
    float atten = 1.0f / (AttenuationCon + AttenuationLin * distToL + AttenuationQuad * pow(distToL, 2));
    const float3 diffuse = DiffuseColor * DiffuseInten * atten * max(0.0f, dot(posToLight, n));
    const float3 w = n * dot(posToLight, n);
    const float3 r = w * 2.0f - posToLight;
    const float3 specular = (DiffuseColor * DiffuseInten) * specularInten * pow(max(0.0f, dot(normalize(r), normalize(worldPos))), specularPower);
